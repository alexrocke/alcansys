        ...
        <section className="relative pt-28 md:pt-36 pb-16 md:pb-24 px-6 md:px-10">
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-12 md:gap-10 items-center relative">
            <div className="md:col-span-6 space-y-7">
              ...
              <p className="text-sm uppercase tracking-[0.25em] text-primary">{hero.tagline}</p>
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className={`h-12 px-7 font-medium tracking-wide ${outlineBtn}`}
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-10 flex flex-col md:flex-row md:items-center gap-6 md:gap-10">
            ...
            <div className="flex flex-wrap items-center gap-3 md:gap-4">
              {(products.items || []).map((p: any) => (
                <span
                  key={p.name}
                  className="flex items-center gap-2 px-4 py-2 rounded-full border border-border bg-background text-foreground/80 hover:text-primary hover:border-primary/50 transition-colors"
                >
                  {p.logo_url ? (
                    <img src={p.logo_url} alt={p.name} loading="lazy" className="h-6 w-auto object-contain" />
                  ) : null}
                  <span className="font-display text-base md:text-lg">{p.name}</span>
                </span>
              ))}
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-12 gap-6 mb-16">
              <div className="md:col-span-5">
                ...
                <h2 className="font-display text-4xl md:text-5xl leading-tight">
                  {services.title}{" "}
                  <em className="text-gradient-ember not-italic">{services.title_highlight}</em>.
                </h2>
              </div>
              <div className="md:col-span-6 md:col-start-7 flex items-end">
                <p className="text-muted-foreground text-lg leading-relaxed">{services.subtitle}</p>
              </div>
            </div>

            <div className="grid md:grid-cols-2 gap-px bg-border rounded-2xl overflow-hidden border border-border">
              {(services.cards || []).map((s: any, i: number) => (
                <div
                  key={s.title}
        ...
        <section id="projetos" className="py-24 md:py-32 px-6 md:px-10 bg-surface border-y border-border">
          <div className="max-w-7xl mx-auto">
            <div className="max-w-3xl space-y-4 mb-14">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">{projects.subtitle}</p>
            </div>
                ...
                >
                  ...
                  <div className="flex flex-col gap-4 p-7 md:p-8 flex-1">
                    ...
                    <div className="flex flex-wrap gap-3 pt-1">
                      <a href={whatsapp} target="_blank" rel="noopener noreferrer">
                        <Button variant="outline" className={`gap-2 ${outlineBtn}`}>
                          Quero algo parecido <ArrowUpRight className="w-4 h-4" />
                        </Button>
                      </a>
                      {p.link ? (
        ...
        <section id="processo" className="py-24 md:py-32 px-6 md:px-10">
          <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-10 md:gap-16">
            <div className="md:col-span-5 md:sticky md:top-32 self-start space-y-6">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">{process.subtitle}</p>
            </div>
        ...
        <section className="py-24 md:py-32 px-6 md:px-10">
          <div className="max-w-6xl mx-auto">
            <h2 className="font-display text-4xl md:text-5xl leading-tight text-center mb-14 max-w-3xl mx-auto">
              {comparativo.title}{" "}
              <em className="text-gradient-ember not-italic">{comparativo.title_highlight}</em>.
            </h2>
            <div className="grid md:grid-cols-2 gap-6">
        ...
        <section id="numeros" className="py-24 md:py-32 px-6 md:px-10 bg-surface border-y border-border">
          <div className="max-w-7xl mx-auto">
            <h2 className="font-display text-4xl md:text-5xl leading-tight text-center mb-16 max-w-3xl mx-auto">
              {experiencia.title}{" "}
              <em className="text-gradient-ember not-italic">{experiencia.title_highlight}</em>.
            </h2>
